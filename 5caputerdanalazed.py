...
def build_features(fs):
    ...
    if fs["bwd_header_len"]:
        ...
        f["Bwd Header Length"] = 20.0   # nothing seen yet
...
def evaluate_flow(fs, log_alert=False):
    ...
    mature_flow = fwd_duration >= FLOW_WARMUP_SECONDS or bwd_count >= 2
    ...
    small_unconfirmed = is_small_unconfirmed_flow(fs, feat, fwd_count, bwd_count, fwd_duration)
    low_signal_one_way = is_low_signal_one_way_flow(fs, feat, fwd_count, bwd_count)
    ...
    if log_alert and (rule_triggered or (ml_prob > 0.92 and benign_score < 3 and fwd_count >= 8)):
        attack_log.append({
...
def get_dashboard_snapshot(log_alerts=False):
    network_state = detect_distributed_attack()

    with flow_lock:
        flows_snapshot = list(flow_stats.values())

    rows = []
    for fs in flows_snapshot:
        row = evaluate_flow(fs, log_alert=log_alerts)
        if row is not None:
            row["network"] = network_state
            rows.append(row)

    if log_alerts and len(attack_log) > 0:
        pd.DataFrame(attack_log).to_csv("attack_log.csv", index=False)

    return pd.DataFrame(rows), network_state
